    ...
    public static function NumbersToWords($number){
        ...
        switch (true) {
            ...
            case $number < 1000:
                ...
                if ($remainder) {
                    $string .= self::$conjunction . self::NumbersToWords($remainder);
                }
                break;
            default:
                ...
                $string = self::NumbersToWords($numBaseUnits) . ' ' . self::$dictionary[$baseUnit];
                if ($remainder) {
                    $string .= $remainder < 100 ? self::$conjunction : self::$separator;
                    $string .= self::NumbersToWords($remainder);
                }
                break;
        }
        return $string;
